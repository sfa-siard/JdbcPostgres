...
public class TestSqlDatabase {
    ...
    public static QualifiedId getQualifiedSimpleType() {
        return new QualifiedId(null, _sTEST_SCHEMA, _sTEST_TYPE_SIMPLE);
    }

    private static final String _sTEST_TYPE_ALL = "TYPSQLALL";

    public static QualifiedId getQualifiedAllType() {
        return new QualifiedId(null, _sTEST_SCHEMA, _sTEST_TYPE_ALL);
    }

    private static final String _sTEST_TYPE_COMPLEX = "TYPSQLCOMPLEX";

    public static QualifiedId getQualifiedComplexType() {
        return new QualifiedId(null, _sTEST_SCHEMA, _sTEST_TYPE_COMPLEX);
    }
    ...
    private static List<TestColumnDefinition> getListCdSimple() {
        ...
        listCdSimple.add(new TestColumnDefinition("CREAL", "REAL", Float.valueOf(Double.valueOf(Math.PI)
                                                                                       .floatValue())));
    ...
    /*------------------------------------------------------------------*/
    public TestSqlDatabase(Connection conn, String sDbUser) throws SQLException {
        _conn = conn;
        _sDbUser = sDbUser;
        try {
    ...
    private void createSchema() throws SQLException {
        ...
        if (iResult != 0) throw new SQLException("Create schema " + _sTEST_SCHEMA + " failed!");
        stmt.close();
        _conn.commit();
    } /* createSchema */
    ...
    private void createTables() throws SQLException {
        createTable(getQualifiedSimpleTable(), _listCdSimple, Arrays.asList(new String[]{_listCdSimple.get(_iPrimarySimple).getName()}), null, null, null);
        createTable(getQualifiedComplexTable(), _listCdComplex, Arrays.asList(new String[]{_listCdComplex.get(_iPrimaryComplex).getName()}), Arrays.asList(new String[]{_listCdComplex.get(_iPrimaryComplex).getName()}), getQualifiedSimpleTable(), Arrays.asList(new String[]{_listCdSimple.get(_iPrimarySimple).getName()}));
    ...
    /*------------------------------------------------------------------*/
    private AttributeDefinition getAttributeDefinition(TestColumnDefinition tcd) throws SQLException {
    ...
    /*------------------------------------------------------------------*/
    private void createType(QualifiedId qiType, List<TestColumnDefinition> _listAd) throws SQLException {
    ...
    /*------------------------------------------------------------------*/
    private void createTable(QualifiedId qiTable, List<TestColumnDefinition> listCd, List<String> listPrimary, List<String> listForeign, QualifiedId qiTableReferenced, List<String> listReferenced) throws SQLException {
        ...
        for (int iColumn = 0; iColumn < listCd.size(); iColumn++) {
            ...
            listTableElements.add(getTableElement(tcd, (tcd.getName()
                                                           .equals("CCHAR_5")) || listPrimary.contains(tcd.getName())));
        }
        if (listPrimary != null) listTableElements.add(getPrimaryTableElement("PK" + qiTable.getName(), listPrimary));
        if (listForeign != null)
            listTableElements.add(getForeignTableElement("FK" + qiTable.getName(), listForeign, qiTableReferenced, listReferenced));
        cts.initTableElements(null, qiTable, listTableElements, null);
        executeCreate(cts.format());
    } /* createTable */

    /*------------------------------------------------------------------*/
    private TableElement getTableElement(TestColumnDefinition tcd, boolean bNotNull) throws SQLException {
        DataType dt = _sf.newDataType();
    ...
    /*------------------------------------------------------------------*/
    private TableElement getForeignTableElement(String sFkName, List<String> listForeign, QualifiedId qiTableReferenced, List<String> listReferenced) {
        IdList ilForeign = new IdList();
        for (int i = 0; i < listForeign.size(); i++)
            ilForeign.get()
                     .add(listForeign.get(i));
        IdList ilReferenced = new IdList();
        for (int i = 0; i < listReferenced.size(); i++)
            ilReferenced.get()
                        .add(listReferenced.get(i));
        TableConstraintDefinition tcd = _sf.newTableConstraintDefinition();
    ...
    private void createView(QualifiedId qiView, List<TestColumnDefinition> listCd, QualifiedId qiTable) throws SQLException {
        ...
        tp.initialize(qiTable, new Identifier(), new ArrayList<Identifier>(), null, false, null, false, false, null, null, false, false);
        ...
        qeb.initialize(null, null, null, null, false, new ArrayList<Identifier>(), null, qs, new QualifiedId(), new ArrayList<TableRowValueExpression>());
        ...
        cvs.initialize(false, qiView, ilColumnNames, new QualifiedId(), new ArrayList<ViewElement>(), new QualifiedId(), qe, false, null);
    ...
    /*------------------------------------------------------------------*/
    private ValueExpression getValueExpression(TestColumnDefinition tcd, List<TestColumnDefinition> listLobs) throws SQLException {
        ValueExpression ve = _sf.newValueExpression();
        CommonValueExpression cve = null;
        Object o = tcd.getValue();
        if ((o == null) || (o instanceof String) || (o instanceof byte[]) || (o instanceof Boolean) || (o instanceof Date) || (o instanceof Time) || (o instanceof Timestamp) || (o instanceof Interval))
            cve = getCommonValueExpressionLiteral();
        else if ((o instanceof BigDecimal) || (o instanceof BigInteger) || (o instanceof Short) || (o instanceof Integer) || (o instanceof Long) || (o instanceof Float) || (o instanceof Double))
            cve = getCommonValueExpressionNumeric();
        else if (o instanceof List<?>) cve = getCommonValueExpression();

        if (o != null) {
            if (o instanceof String) {
                String s = (String) o;
                if ((s != null) && (!tcd.getType()
                                        .startsWith(PreType.CLOB.getKeyword())) && (!tcd.getType()
                                                                                        .startsWith(PreType.NCLOB.getKeyword())) && (s.length() < 1000))
                    cve.getValueExpressionPrimary()
                       .getUnsignedLit()
                ...
                byte[] buf = (byte[]) o;
                if ((buf != null) && (!tcd.getType()
                                          .startsWith(PreType.BLOB.getKeyword())) && (buf.length < 1000))
                    cve.getValueExpressionPrimary()
                       .getUnsignedLit()
                       .initBytes(buf);
                else {
                ...
                try {
                    @SuppressWarnings("unchecked") List<TestColumnDefinition> listCd = (List<TestColumnDefinition>) o;
                    if (listCd.size() == 4) {
                        // ARRAY
    ...
    /*------------------------------------------------------------------*/
    private SqlArgument getAttributeValue(TestColumnDefinition tcd, List<TestColumnDefinition> listLobs) throws SQLException {
    ...
    /*------------------------------------------------------------------*/
    private UpdateSource getUpdateSource(TestColumnDefinition tcd, List<TestColumnDefinition> listLobs) throws SQLException {
        UpdateSource us = _sf.newUpdateSource();
        if (tcd.getValue() != null) {
            ValueExpression ve = getValueExpression(tcd, listLobs);
            us.initialize(ve, null);
        } else us.initialize(null, SpecialValue.NULL);
        return us;
    } /* getUpdateSource */
