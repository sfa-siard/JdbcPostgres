...
public class TestPostgresDatabase {
    ...
    public static void grantSchemaUser(Connection conn, String sSchema, String sDbUser) throws SQLException {
    ...
    public static void revokeSchemaUser(Connection conn, String sSchema, String sDbUser) throws SQLException {
        Statement stmt = conn.createStatement();
        ...
        public String getValueLiteral() {
            ...
            if (_oValue != null) {
                if (getName().equals("CBYTEA"))
                    ...
                    @SuppressWarnings("unchecked") List<ColumnDefinition> listCd = (List<ColumnDefinition>) getValue();
                    ...
                    StringBuilder sb = new StringBuilder("{");
                    @SuppressWarnings("unchecked") List<ColumnDefinition> listRowCd = (List<ColumnDefinition>) getValue();
                    for (int iRow = 0; iRow < listRowCd.size(); iRow++) {
                        ColumnDefinition cdRow = listRowCd.get(iRow);
                        if (iRow > 0) sb.append(", ");
                        sb.append("{");
                        @SuppressWarnings("unchecked") List<ColumnDefinition> listColumnCd = (List<ColumnDefinition>) cdRow.getValue();
                        for (int iColumn = 0; iColumn < listColumnCd.size(); iColumn++) {
                            ColumnDefinition cdColumn = listColumnCd.get(iColumn);
                            if (iColumn > 0) sb.append(", ");
    ...
    private static List<TestColumnDefinition> getCdSimple() {
        ...
        listCdSimple.add(new ColumnDefinition("CREAL", PostgresType.REAL.getKeyword(), new Float(Double.valueOf(Math.PI)
                                                                                                       .floatValue())));
    ...
    public TestPostgresDatabase(PostgresConnection connPostgres, String sDbUser) throws SQLException, IOException {
    ...
    private void dropType(QualifiedId qiType) {
        if (qiType.getName()
                  .equals(_sTEST_INTEGER_DOMAIN)) executeDrop("DROP DOMAIN " + qiType.format());
        else executeDrop("DROP TYPE " + qiType.format());
    } /* dropType */
    ...
    private void createTable(QualifiedId qiTable, List<TestColumnDefinition> listCd, List<String> listPrimary, List<String> listUnique) throws SQLException {
        ...
        for (int iColumn = 0; iColumn < listCd.size(); iColumn++) {
            ...
            sbSql.append(" ");
            if (cd.getName()
                  .equals("CINT_BUILTIN")) sbSql.append("int4range");
            else if (cd.getName()
                       .equals("CSTRING_ARRAY")) sbSql.append("text[4]");
            else if (cd.getName()
                       .equals("CDOUBLE_MATRIX")) sbSql.append("float8[3][4]");
            else sbSql.append(cd.getType());
        }
        if (listPrimary != null) {
            sbSql.append(",\r\n  CONSTRAINT PK");
    ...
    private void insertTables() throws SQLException, IOException {
        insertTable(getQualifiedSimpleTable(), _listCdSimple);
        insertTable(getQualifiedComplexTable(), _listCdComplex);
    } /* insertTables */

    private void insertTable(QualifiedId qiTable, List<TestColumnDefinition> listCd) throws SQLException, IOException {
        ...
        for (int iColumn = 0; iColumn < listCd.size(); iColumn++) {
            ...
            if (cd.getValue() != null) {
                ...
                if (sLiteral.length() < 100000) sbSql.append(sLiteral);
                else {
                    sbSql.append("?");
        ...
        for (int iLob = 0; iLob < listLobs.size(); iLob++) {
            ...
            } else throw new SQLException("Invalid LOB type " + o.getClass()
                                                                 .getName() + "!");
        }
        int iResult = pstmt.executeUpdate();
        Assert.assertSame("Insert failed!", 1, iResult);
        pstmt.close();
        _conn.commit();
    } /* insertTable */
